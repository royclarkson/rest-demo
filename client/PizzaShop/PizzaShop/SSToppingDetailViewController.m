...
//
//
//  SSToppingDetailViewController.m
//  PizzaShop
//
...
@interface SSToppingDetailViewController ()
...
@implementation SSToppingDetailViewController

- (IBAction)saveData:(id)sender
{
    self.topping.name = self.textName.text;
    
    NSURL *url = [SSConnectionSettings urlWithFormat:@"/toppings"];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:url];
    NSError *jsonError;
    NSData *postData = [NSJSONSerialization dataWithJSONObject:[self.topping dictionaryValue] options:0 error:&jsonError];
...
- (void)saveDataDidFinishWithData:(NSData *)data
{
    ...
    self.topping = [[SSTopping alloc] initWithDictionary:dictionary];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.topping)
    {
        self.textName.text = self.topping.name;
    }
}
